...
export class PantryMainComponent {
  ...
  editCategory = new FormControl<Category>(DEFAULT_CATEGORY, {
    nonNullable: true,
  });
  ...
  filteredGrouped$ = combineLatest([
    ...
    map(([items, q]) => {
      ...
      const sortedCats = Array.from(groups.keys()).sort((a, b) => {
        const ia = order.has(a as Category)
          ? (order.get(a as Category) as number)
          : 999;
        const ib = order.has(b as Category)
          ? (order.get(b as Category) as number)
          : 999;
  ...
  startEdit(item: PantryItem) {
    ...
    this.editCategory.setValue(
      CATEGORIES.includes(cat) ? cat : DEFAULT_CATEGORY,
    );
  }
